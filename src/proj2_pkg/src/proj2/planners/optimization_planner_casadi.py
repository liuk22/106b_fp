...
def bicycle_robot_model(q, u, L=0.3, dt=0.01):
    ...
    theta = q[2]
    phi = q[3]
    q_dot = vertcat(cos(theta), sin(theta), tan(phi) / L, 0) * u[0] + vertcat(0, 0, 0, 1) * u[1]
    return q + q_dot * dt
...
def initial_cond(q_start, q_goal, n):
    ...
    # Your code here.
    q0 = np.zeros((4, n+1))
    q0[0] = np.linspace(q_start[0], q_goal[0], num=n+1)
    q0[1] = np.linspace(q_start[1], q_goal[1], num=n+1)
    q0[2] = np.linspace(q_start[2], q_goal[2], num=n+1)
    q0[3] = np.linspace(q_start[3], q_goal[3], num=n+1)
    u0 = np.zeros((2, n)) 
...
def objective_func(q, u, q_goal, Q, R, P):
    ...
    for i in range(n):
        ...
        term = mtimes((qi - q_goal).T , mtimes(Q , (qi - q_goal))) 
        term += mtimes(ui.T, mtimes(R , ui))
        obj += term

    # Define the last term here: (q(N+1) - q_goal)^T * P * (q(N+1) - q_goal)
    term_last = mtimes((q[:, n] - q_goal).T, mtimes(P, (q[:, n] - q_goal)))
    obj += term_last
    return obj
...
def constraints(q, u, q_lb, q_ub, u_lb, u_ub, obs_list, q_start, q_goal, L=0.3, dt=0.01):
    ...
    for t in range(q.shape[1] - 1):
        ...
        q_tp1 = q[:, t + 1]
        u_t   = u[:, t]
        F_t = bicycle_robot_model(q_t, u_t, L=L, dt=dt)
        constraints.append(q_tp1 - F_t == 0) # You should use the bicycle_robot_model function here somehow.

    # Obstacle constraints
    for obj in obs_list:
        obj_x, obj_y, obj_r = obj
        for t in range(q.shape[1]):
            dist = (q[0, t] - obj_x) ** 2 + (q[1, t] - obj_y) ** 2
            constraints.append(dist >= obj_r **2) # Define the obstacle constraints.
